...
class TestResults(unittest.TestCase):
    ...
    def create_game(self):
        ...
        return json_returned['id']

    def test_upload_n_load_results(self):
        request = webapp2.Request.blank('/1/upload_results/%s' % URLSAFE)
        request.method = 'POST'
        request.body = "results=%s" % SOME_RES
        response = request.get_response(main.app)
        self.assertEqual(response.status_int, 200)
        request = webapp2.Request.blank('/1/get_results/%s' % URLSAFE)
        request.method = 'GET'
        response = request.get_response(main.app)
        self.assertEqual(response.status_int, 200)
    ...
    def test_pregame_upload_n_check_res(self):
        # create 2 games:
        game1_id = self.create_game()
        game2_id = self.create_game()

        # check for res, return empty str:
        request = webapp2.Request.blank('1/check_for_results/0')
        request.method = 'GET'
        response = request.get_response(main.app)
        self.assertEqual(response.body, '')

        # upload some res from game1:
        request = webapp2.Request.blank('/1/upload_results/%s' % game1_id)
        request.method = 'POST'
        request.body = "results=%s" % SOME_RES
        response = request.get_response(main.app)
        self.assertEqual(response.status_int, 200)
        ...
        request = webapp2.Request.blank('/1/upload_results/%s' % game2_id)
        request.method = 'POST'
        request.body = "results=%s" % SOME_RES
        response = request.get_response(main.app)
        self.assertEqual(response.status_int, 200)
        ...
        request = webapp2.Request.blank('2/check_for_results/0')
        request.method = 'GET'
        response = request.get_response(main.app)
        results = json.loads(response.body)
        ...
        request = webapp2.Request.blank('1/check_for_results/%s' % timestamp)
        request.method = 'GET'
        response = request.get_response(main.app)
        results = json.loads(response.body)
        self.assertEqual(len(results), 1)
        ...
        request = webapp2.Request.blank('3/check_for_results/0')
        request.method = 'GET'
        response = request.get_response(main.app)
        results = json.loads(response.body)
    ...
    def test_upload_log(self):
        request = webapp2.Request.blank('/1/upload_log/%s' % URLSAFE)
        request.method = 'POST'
